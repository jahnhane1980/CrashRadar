...
async function runAllIndicatorsGrandPrix() {
  ...
  fs.writeFileSync(
    path.resolve(process.cwd(), 'data/cache/macro/grand_prix_all_indicators_results.json'),
    JSON.stringify({ summary, details: Array.from(indResults.values()) }, null, 2),
    'utf8'
  );

  console.log('[Erfolg] Alle Ergebnisse in data/cache/macro/grand_prix_all_indicators_results.json gespeichert.');
  await fe.close();
}
