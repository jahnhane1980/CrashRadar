...
### 6. Kritische Erkenntnis: SPY-Modell (v1 / timeSteps=5)
Ein erster Testlauf des universellen SPY-Modells (`spy_regime_v1` mit Default-Settings) ergab gravierende Schwächen an Wendepunkten:
* **Tops (Melt-Ups):** Werden komplett verfehlt (z.B. Covid-Top 2020: 99.7% `BULL_MARKET`).
* **Bottoms (V-Shapes):** Werden nicht als `CYCLE_BOTTOM` erkannt (z.B. Covid-Bottom 2020: 100% `BEAR_MARKET`). Das Modell agiert wie ein Retail-Trader in Panik.
* **Ursache:** Die Default-Konfiguration in `ML-Config.json` nutzt `timeSteps: 5`. Ein LSTM-Gedächtnis von 5 Tagen ist viel zu kurz, um langfristige zyklische Wendepunkte zu erkennen. Das Modell verkommt zu einem simplen, stark nachlaufenden Momentum-Tracker, da die Features (wie RSI) an Böden ohnehin massiv überverkauft sind.
* **🔥 NÄCHSTES TODO (Höchste Prio):** Anpassung der `ML-Config.json`. Die `timeSteps` für SPY/QQQ müssen drastisch erhöht werden (z. B. auf 14 oder 21 Tage), und das Modell muss zwingend neu trainiert werden, bevor es als "Bottom-Fischer" in die Live-Engine (`IndicatorEngine.js`) integriert wird.

### 7. Palantir (PLTR) - ML Model Evaluation (Juli 2026)
Beim Trainieren und Testen des 7-Klassen-Modells für Palantir (`Log_Return_EMA3`, `Volume_Z_Score` als neue Features) zeigte sich im Test auf den Zeitraum Juni/Juli 2026 eine signifikante Divergenz:
* Der sture **Dow-Theorie RegimeLabeler** wertete den jüngsten Absturz von $136 auf $107 (Bruch SMA-50) aufgrund der starren Drawdown-Mathematik sofort als `BEAR_MARKET`.
* Das **ML-Modell** verweigerte dieses Label und wertete die Situation souverän als `BULL_CORRECTION`.

**Erkenntnis:** Bei massiv volatilen Growth-Aktien wie PLTR (die aus einer extremen `BASE` von $6 kamen) ist nach einem 1.5-jährigen Bull-Run nicht direkt klar, ob ein drastischer Rücksetzer der Start eines echten Bärenmarkts oder nur eine scharfe Korrektur ist. Das ML-Modell generalisiert hier die Volatilität der Aktie und warnt vor vorzeitigen Bärenmarkt-Signalen.
**To-Do:** Diese Divergenz (und die Performance des PLTR-Modells) muss später im Jahr nochmals getestet werden, um retrospektiv zu validieren, ob das ML-Netz oder der algorithmische Labeler Recht behielt.

### 8. Einzelaktien-Konfiguration (Growth: SOFI, ZETA, NVTS)
Die Champion-Konfiguration von PLTR (7 Klassen inkl. `BASE`, Nutzung von `Log_Return_EMA3`, `Volume_Z_Score`, `Dist_52w_High/Low`) hat sich als exzellente Schablone für volatile, nicht-Megacap Growth-Titel erwiesen. Da alle Titel ähnliche Hype- und Volatilitätszyklen durchlaufen, wird diese Architektur universell für die weiteren Growth-Ticker übernommen.

**Evaluierungs-Ergebnisse (Live-Test vs. Dow-Theorie):**
Nach dem Training wurden SOFI, ZETA und NVTS an historischen Wendepunkten getestet. Das Ergebnis zeigt starke aktienspezifische Bias-Muster aufgrund der hohen Volatilität und kurzen Historie (< 1200 Tage):
* **SOFI (25.0% Trefferquote):** Hat einen extremen Bull-Bias erlernt. Das Netz stuft selbst harte Bärenmärkte oft verharmlosend als `BULL_CORRECTION` ein. Klassische Bullenmärkte werden jedoch mit 99% Konfidenz sauber erkannt.
* **ZETA (50.0% Trefferquote):** Das ausbalancierteste Modell dieser Gruppe. Erkannte historische Bullen- sowie tiefe Bärenmärkte exakt richtig (jeweils >99% Konfidenz). Schwächelt lediglich bei der präzisen Identifikation der `BASE`-Übergänge.
...
LSTMs neigen bei hochvolatilen, stark geshorteten Einzelwerten dazu, die primäre historische Stimmung der Aktie (Dauer-Bullish bei SOFI, Dauer-Bearish bei NVTS) zu "memorieren". Um dieses Rauschen zu durchbrechen und echte Kausalität herzustellen, ist die geplante Integration der **FINRA Short-Volume Daten** als Feature für diese Modelle zwingend erforderlich.
...
> 🛡️ **Das strikte Regelwerk zur Priorisierung der KI und der Veto-Alarme ist definiert unter [docs/VETO_LOGIC.md](file:///C:/GitHub/CrashRadar/docs/VETO_LOGIC.md).**
