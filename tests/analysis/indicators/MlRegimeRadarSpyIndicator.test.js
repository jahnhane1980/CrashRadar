...
import { MlRegimeRadarSpyIndicator } from '../../../src/analysis/indicators/MlRegimeRadarSpyIndicator.js';

describe('MlRegimeRadarSpyIndicator', () => {
    const indicator = new MlRegimeRadarSpyIndicator();

    it('should have correct name and category', () => {
        expect(indicator.name).toBe('ML Regime Radar (SPY)');
    ...
    it('chaos: should fallback to 0.0% if confidence is missing', () => {
        const result = indicator.evaluate([{ mlRegimeSpy: { phase: 'UPTREND' } }]);
        expect(result.status).toBe('OK');
        expect(result.value).toBe('BULL (0.0%)');
    });

    it('chaos: should output negative percentage for negative confidence', () => {
        const result = indicator.evaluate([{ mlRegimeSpy: { phase: 'UPTREND', confidence: -0.2 } }]);
        expect(result.status).toBe('OK');
