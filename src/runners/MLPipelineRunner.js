...
export class MLPipelineRunner {
  ...
  async run(step = 'all') {
    ...
    Logger.info(`📋 Ausgewählte Phase: ${step}\n`);

    try {
      // --- PHASE 1: FEATURE ENGINEERING (ETL) ---
      if (step === 'features' || step === 'all') {
        Logger.info(`=== PHASE 1: Feature Engineering (ETL) ===`);
        ...
        if (fs.existsSync(customPath)) {
           Logger.info(`[MLPipelineRunner] 🔧 Spezialisierter FeatureBuilder für ${this.ticker} gefunden. Nutze diesen anstelle von Default.`);
           ...
           Logger.info(`[MLPipelineRunner] 🔧 Nutze universellen DefaultFeatureBuilder.`);
        }
        
        const builder = new FeatureBuilderClass(this.ticker, this.repo, this.config);
        await builder.build();
        Logger.info(`=== PHASE 1: ERFOLGREICH ABGESCHLOSSEN ===\n`);
      }

      // --- PHASE 2: MODEL TRAINING (LSTM) ---
      if (step === 'train' || step === 'all') {
        Logger.info(`=== PHASE 2: Model Training (LSTM) ===`);
        const trainer = new ModelTrainer(this.ticker, this.config);
        await trainer.train();
        Logger.info(`=== PHASE 2: ERFOLGREICH ABGESCHLOSSEN ===\n`);
      }
