...
* 📄 **[`Kamikaze-Growth.md`](file:///D:/GitHub/CrashRadar/docs/architecture/strategies/Kamikaze-Growth.md):**  
  *50/50 Kamikaze-Growth-Strategie auf nativer US-Dollar-Basis (~94.000 $ Realdepot-Pool): 50 % Tech Sub-Bucket & 50 % Krypto-Equity Sub-Bucket im S&P 500 Mutterschiff, User-kuratierte Watchlist (`OBSERVE`), BTC-USD 21-Wochen-EMA Master-Regime mit 40/30/30-Pyramide, 'Kein Kauf ohne SEC-10-Q'-Fundamental-Türsteher, 100 % Notfall-Evakuierung (50 % Gold / 50 % Cash), **Live-Broker-Anbindung (Trading-Konto Ingestion & Discretionary Override: 'Broker-Realität ist Gesetz')**, **Read-Only Telegram Broadcast** für Follower sowie **RFC zum Zwei-Phasen-Restrukturierungsmodell (Oktober-Liquidierung & 3 Investment-Typen: LASTING_HOLD, CYCLICAL, BINARY)** inkl. PoC in [`KamikazeGrowthSimulation.js`](file:///D:/GitHub/CrashRadar/scratch/architecture/strategies/KamikazeGrowthSimulation.js).*
* 📄 **[`Kamikaze-Stock-Radar.md`](file:///D:/GitHub/CrashRadar/docs/architecture/strategies/Kamikaze-Stock-Radar.md):**  
  *Spezifikation des Kamikaze Stock-Radars: User-kuratierte Watchlist (`OBSERVE` Pflicht & `firstSeenDate`), Weinstein Stage-2 Türsteher, SEC 10-Q Fundamental-Gate, Status `HOLD & BUY` mit Verkaufsblockade, Sektor-Relativität, 3-Stufen-Abbau, die 4 Turnaround- & Climax-Top-Erkenntnisse (NVTS, IBRX, S) sowie **RFC-Schnittstelle für Investment-Typen (`LASTING_HOLD` vs. `CYCLICAL` vs. `BINARY`)**.*
...
* 📄 **[`Stock-Radar-Turnaround-Framework.md`](file:///D:/GitHub/CrashRadar/docs/architecture/strategies/Stock-Radar-Turnaround-Framework.md):**  
  *Produktions-Architektur des Stock-Radars für das Kamikaze-Portfolio: Vollständiger Lebenszyklus für abgestrafte Qualitäts-Wachstumswerte (Small/Mid-Caps wie IBRX, NVTS, S, SOFI, PLTR) und Big Tech (META, NFLX, NOW) – von der 4–6-Jahres-Kater-Filterung und dem fundamentalen Solvenz-Airbag über die 3-Phasen-Bodenbildung, den Institutional Event-Pivot ($t_0$), $t_0$-Anchored Rolling Blöcke (3D/5D/21D) und adaptive Dip-DNA bis zum 2-Stufen-Exit (+71 %-Punkte Mehrertrag an parabolischen Spitzen vor Peak-Kollapsen).*
* 📄 **[`Post-Ipo-Growth-Engine.md`](file:///D:/GitHub/CrashRadar/docs/architecture/strategies/Post-Ipo-Growth-Engine.md):**  
  *System-Architektur & Master-Spezifikation (V2-Zukunftsprojekt): Automatisierte Markt-Screening-Pipeline für qualitative Wachstums- und Turnaround-Aktien im Reife- und Kater-Zeitfenster von 1 bis 5 Jahren nach Börsengang (SIC/NAICS-Filter, De-SPAC Super 8-K Sonderklausel, Runway-Formel, Rule of 40, Verwässerungs-Matrix, 2027-Skalierungsmodell für 300 Ticker im Freetier, SEC EDGAR Facts API und Supabase Cloud-Signal-Hub).*
