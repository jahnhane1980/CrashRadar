...
1. **Training & Forschung (Python-Pipeline):**  
   * Skript unter [`scratch/ml/train_macro_regime.py`](file:///D:/GitHub/CrashRadar/scratch/) unter Nutzung von `xgboost`, `scikit-learn` und `shap`.
...
2. **Ausführung in CrashRadar (Node.js Engine):**  
   * Service [`src/services/MacroMlService.js`](file:///D:/GitHub/CrashRadar/src/services/) lädt das JSON-Tree-Modell oder verwendet `onnxruntime-node`.
...
```bash
node scratch/analyse/Historical-Event-Raw-Audit.js
```
...
```bash
python scratch/ml/train_macro_regime.py
```
* **Output:**  
...
```bash
node scratch/ml/evaluate_macro_model.js
```
...
Die Auswertung in Node.js ([`scratch/ml/evaluate_macro_model.js`](file:///D:/GitHub/CrashRadar/scratch/ml/evaluate_macro_model.js)) liefert folgende Kennzahlen:
