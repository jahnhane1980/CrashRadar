...
* **Statistische Realität (Aktien):** Über rollierende 3- bis 6-Monats-Fenster liegt die Korrelation zwischen S&P 500 und Net Liquidity bei mickrigen **0.05** *(siehe [scratch/research/macro-proofs/Net-Liquidity-Illusion.js](file:///D:/GitHub/CrashRadar/scratch/research/macro-proofs/Net-Liquidity-Illusion.js))*.
...
* **Statistische Realität:** Die 6-Monats-Korrelation zwischen dem S&P 500 und dem DXY liegt bei starken **-0.31** *(siehe [scratch/research/macro-proofs/Financial-Conditions-DXY.js](file:///D:/GitHub/CrashRadar/scratch/research/macro-proofs/Financial-Conditions-DXY.js))*.
...
* **Statistische Realität:** In den 6 Monaten vor historischen Aktien-Crashes (z.B. 2018, 2020, 2025) tendierte der Anleihenmarkt (TLT) **seitwärts oder sogar negativ** (-0.4% bis -9.5%). Es gibt keine massive Umschichtung *vor* dem Panik-Event *(siehe [scratch/research/macro-proofs/Bonds-Illusion.js](file:///D:/GitHub/CrashRadar/scratch/research/macro-proofs/Bonds-Illusion.js))*.
...
* **Gold als verlässlicher "Boden-Indikator":** Die wichtigste Erkenntnis ist das Timing der Erholung. **In 100% der betrachteten Crashes (6 von 6)** erreichte Gold seinen absoluten Tiefpunkt und drehte wieder nach oben, **bevor** der S&P 500 (SPY) seinen Tiefpunkt fand *(siehe [scratch/research/macro-proofs/Copper-Gold-Commodities.js](file:///D:/GitHub/CrashRadar/scratch/research/macro-proofs/Copper-Gold-Commodities.js))*.
...
* **Der Öl-Mythos (Die Miner-Margen):** Es gibt den Mythos, dass ein steigender Ölpreis die Margen der Goldminen auffrisst und sie dem Goldpreis hinterherhinken lässt. Ein Test *(siehe [scratch/research/macro-proofs/GDX-vs-Oil.js](file:///D:/GitHub/CrashRadar/scratch/research/macro-proofs/GDX-vs-Oil.js))* straft das Lügen: Wenn Gold in einen Bullenrun (>10%) übergeht und Öl *fällt*, performt GDX das Gold um durchschnittlich +9,42% aus. Wenn Öl im selben Zeitraum massiv *steigt* (>20%), performt GDX das Gold immer noch um +8,90% aus. Fazit: Wenn die absolute Gold-FOMO kickt, ignorieren Investoren die Bagger-Kosten komplett. GDX wird ausschließlich als Hebel auf Gold gehandelt.
...
* **Empirischer Beweis (Der Stealth-Exit Test):** Ein Backtest ([`scratch/research/macro-proofs/test_stealth_exit_theory.js`](file:///D:/GitHub/CrashRadar/scratch/research/macro-proofs/test_stealth_exit_theory.js)) mit realen, historischen SqueezeMetrics-Daten (2011-2026) hat die Dark-Pool-Divergenz nun empirisch bewiesen.
...
* **Central Bank Policy Error (DFF vs T10YIE vs DXY):** Das mathematisch perfekte Kaufsignal für Gold (Fiat-Flucht). Wenn die FED über ein 60-Tage-Fenster panisch die Zinsen senkt (DFF sinkt > 0.25%), der Anleihemarkt aber gleichzeitig steigende Inflationserwartungen einpreist (T10YIE steigt > 0.10%), verliert das Fiat-System massiv an Vertrauen. Der Indikator triggert **nur dann**, wenn der US-Dollar (DXY) im selben Zeitraum nicht stark aufwertet (<= +2,0%), da ein starker Dollar den Gold-Run blockiert.
  * *Statistische Realität:* Mit dem DXY-Filter stieg Gold in 73,6% dieser Phasen an. Die durchschnittliche 60-Tage-Rendite für Gold in diesen Mega-Runs beträgt extreme **+7,95%** *(siehe [scratch/research/macro-proofs/Central-Bank-Policy-Error.js](file:///D:/GitHub/CrashRadar/scratch/research/macro-proofs/Central-Bank-Policy-Error.js))*.
...
* **Statistische Häufigkeit von Erleichterungsrallyes:**
  * **In 100 % der Fälle (4 von 4 in unserer DB ab 2000)**, in denen die Fed im September pausierte oder die Zinsen senkte (2002, 2006, 2010, 2014), folgte im 4. Quartal eine **durchschnittliche S&P 500 Rallye von +7,10 %** *(siehe empirischer Backtest [scratch/research/macro-proofs/Midterm-Fed-Seasonality.js](file:///D:/GitHub/CrashRadar/scratch/research/macro-proofs/Midterm-Fed-Seasonality.js))*.
...
* **Margin Debt (FINRA Wertpapierkredite):** Das absolut reinste Fieberthermometer für "Gier und Hebel" im US-Aktienmarkt. Margin Debt misst, wie viel Geld sich Anleger auf Pump leihen, um Aktien zu kaufen.
  * *Signal (Top):* Margin Debt erreicht immer ein historisches Allzeithoch und dreht (stagniert oder fällt) **2 bis 6 Monate bevor** der S&P 500 seinen absoluten Höhepunkt erreicht *(siehe [scratch/research/macro-proofs/Margin-Debt-Top.js](file:///D:/GitHub/CrashRadar/scratch/research/macro-proofs/Margin-Debt-Top.js))*. Das "Smart Money" und die Kreditlinien sind erschöpft, bevor der Retail-Markt sein finales Blow-Off-Top bildet. Da dieser Vorlauf sehr groß ist, stellt die ca. 3-wöchige Veröffentlichungsverzögerung der FINRA hier kein Problem dar.
...
* **Empirischer Beweis:** 
  * Das exakte Timing ohne Verzögerung wurde durch das Backtest-Skript [wresbal-timing-test.js](file:///D:/GitHub/CrashRadar/scratch/trash/wresbal-timing-test.js) validiert.
  * Das Scheitern von kleinen Liquiditätsspritzen (die Ausnahmen ohne vorherige Panik) wurde im Skript [wresbal-failure-test.js](file:///D:/GitHub/CrashRadar/scratch/trash/wresbal-failure-test.js) empirisch bewiesen.
  * Das komplette 4-Phasen-Konstrukt (TGA-Warnung -> Crash -> BORROW-Panik -> WRESBAL-Rettung) ist als State-Machine im Skript [backtest-indicator.js](file:///D:/GitHub/CrashRadar/scratch/research/macro-proofs/backtest-indicator.js) und [backtest-recent.js](file:///D:/GitHub/CrashRadar/scratch/research/macro-proofs/backtest-recent.js) simuliert.
  * Zu den Simulationsergebnissen existieren vollständige Test-Logs unter [Indicator-Trigger-Report.md](file:///D:/GitHub/CrashRadar/scratch/research/macro-proofs/Indicator-Trigger-Report.md).
...
* 🔴 **Ausschluss-Kriterium: Sahm Rule & BLS-Daten:** Wurden historisch als massiv nachlaufend (lagging) entlarvt (siehe [`scratch/research/macro-proofs/Sahm-Rule.js`](file:///D:/GitHub/CrashRadar/scratch/research/macro-proofs/Sahm-Rule.js)) und dürfen **nicht** als Frühwarnsystem genutzt werden!
...
* **Die Arbeitsmarkt-Divergenz (Quantität vs. Qualität):** Während die Headline-Daten trügen, liefert die innere Struktur der Arbeitsmarktdaten ein klares, vorausschauendes Signal.
  ...
  * *Beweis & Backtest:* Ein historischer Test über alle Crashes seit 1995 ([analyze_labor_market.js](file:///D:/GitHub/CrashRadar/scratch/research/macro-proofs/analyze_labor_market.js) und [Crash-Arbeitsmarkt-Analyse.md](file:///D:/GitHub/CrashRadar/docs/research/macro-proofs/Crash-Arbeitsmarkt-Analyse.md)) belegt die Treffsicherheit dieser Divergenzen. Nur bei reinen Bewertungsblasen (Dotcom) reagierte die Realwirtschaft verzögert.
...
* **Challenger, Gray & Christmas Report:** Im Gegensatz zu den staatlichen Nachzüglern (wie Sahm Rule oder JOLTS) ist der monatliche Challenger-Report (Ankündigungen von Entlassungen durch CEOs) ein echter Frühindikator. CEOs wissen, dass die Wirtschaft kippt, bevor es in der staatlichen Statistik auftaucht.
  * *Beweis & Backtest:* Ein historischer Backtest über alle Crashes seit 1999 ([Challenger-Gray-Christmas-Report-Leading-Indicator.js](file:///D:/GitHub/CrashRadar/scratch/research/macro-proofs/Challenger-Gray-Christmas-Report-Leading-Indicator.js)) belegt die Prognosekraft für **deflationäre/wirtschaftliche Crashes**.
...
* **Säule 2: Gamma-Hedging am Optionsmarkt:** Institutionen handeln Optionen. Market Maker müssen diese Positionen über die Aktie absichern. Kennt man die Open-Interest-Grenzmarken (Strikes), kann man die algorithmischen Käufe/Verkäufe der Market Maker vorhersehen. 
  * *Status (Live-Recording):* Wir nutzen `yahoo-finance2` (siehe [Machbarkeitsstudie](file:///D:/GitHub/CrashRadar/scratch/research/macro-proofs/Spurenlesen.js)), um täglich die Options Chains abzurufen. **Wichtig:** Da Yahoo keine historischen Optionsketten anbietet, können wir dieses Modell aktuell noch nicht backtesten. Wir zeichnen die Live-Daten (die aggregierten Open-Interest-Cluster der nächsten 30-45 Tage) ab jetzt täglich in unserer Datenbank auf, um uns einen eigenen historischen Datensatz für spätere Backtests aufzubauen.
...
* **Die Überprüfung:** Ein Test-Skript (siehe [`scratch/research/methodology-audits/Noise-Test-IndicatorEngine.js`](file:///D:/GitHub/CrashRadar/scratch/research/methodology-audits/Noise-Test-IndicatorEngine.js)) fügt historischen SPY-Kursen zufälliges Rauschen (+/- 1% pro Tag) hinzu. Laufen die Crash-Signale der `IndicatorEngine.js` für 2008 und 2020 weiterhin stabil, ist das System robust. Zerbricht die Performance, liegt Overfitting vor.
...
* **Die Überprüfung:** Der historische Beweis wurde über das Backtest-Skript [Fractional-Kelly.js](file:///D:/GitHub/CrashRadar/scratch/research/macro-proofs/Fractional-Kelly.js) erbracht.
...
* **Die Überprüfung:** Der historische Beweis wurde über das Backtest-Skript [RVOL-Breakout-These.js](file:///D:/GitHub/CrashRadar/scratch/architecture/single-asset-radar/RVOL-Breakout-These.js) angetreten.
...
* **Die Überprüfung:**
  * **Empirische Auswertung Zinsinversion & Schuldenzyklus (1970–2026):** Eine historische Event-Studie über 7 Zyklen seit 1970 (siehe Quant-Backtest [`scratch/research/dalio-cycles/validate_dalio_thesis.py`](file:///D:/GitHub/CrashRadar/scratch/research/dalio-cycles/validate_dalio_thesis.py)) beweist:
  ...
  * **Backtest "3 von 4 Indikatoren ROT" & Kipppunkt-Kaskade (1970–2026):** Der historische Backtest aller Marktphasen seit 1970 (siehe Quant-Backtest [`scratch/research/dalio-cycles/test_3_of_4_system.py`](file:///D:/GitHub/CrashRadar/scratch/research/dalio-cycles/test_3_of_4_system.py)) untersucht das gleichzeitige Anschlagen von 3 von 4 Makro-Bedingungen (Zinsdruck, Zinsinversion, Kreditrisiko, Schuldenquote):
