...
Dieses Dokument sammelt theoretische Konzepte und Feature-Ideen, die in zukünftige `<Ticker>FeatureBuilder` (siehe [`docs/architecture/ml/ML_ARCHITECTURE.md`](file:///D:/GitHub/CrashRadar/docs/architecture/ml/ML_ARCHITECTURE.md)) integriert werden könnten, um die Schwächen des aktuellen LSTMs (insbesondere bei hochvolatilen Growth-Aktien) zu beheben.
