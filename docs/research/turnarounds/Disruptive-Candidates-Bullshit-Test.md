...
```mermaid
flowchart TD
    subgraph Phase1 ["1. Sektorale Liquidierung"]
        NVDA["NVIDIA / Halbleiter-Top\n(DSI > 118 Tage, Inventar +438%)"] --> Flush["Index-Kaskade & CapEx-Stopp\n(Hyperscaler kürzen Ausgaben)"]
    end
    
    subgraph Phase2 ["2. Der Anti-KI-Korb"]
        Flush --> Basket["Institutioneller Korb-Abverkauf:\n'KI bringt keinen schnellen ROI!'"]
        Basket --> TEM_Dump["TEM (-50% Multiple-Crash)\n'Zu teuer bewertet!'"]
        Basket --> SYM_Dump["SYM (Hardware/CapEx Pause)\n'Lager-Neubauten auf Eis!'"]
        Basket --> PATH_Dump["PATH (Rückfall auf Doppelboden)\n'SaaS-Multiples schrumpfen!'"]
        Basket --> RXRX_Dump["RXRX (Nvidia-Beta-Absturz)\n'Unprofitabler KI-Zock!'"]
    end
    
    subgraph Phase3 ["3. Die Entkopplung"]
        CRSP_Safe["CRSP (CRISPR)\nUnkorreliert zu Chips & CapEx\nBodenbildung unabhängig von NVDA!"]
    end
```
