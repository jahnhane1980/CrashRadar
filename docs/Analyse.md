...
* **Statistische Realität:** Die 6-Monats-Korrelation zwischen dem S&P 500 und dem DXY liegt bei starken **-0.49**.
...
* **Statistische Realität:** In den 6 Monaten vor historischen Aktien-Crashes (z.B. 2018, 2020, 2025) tendierte der Anleihenmarkt (TLT) **seitwärts oder sogar negativ** (-0.4% bis -9.5%). Es gibt keine massive Umschichtung *vor* dem Panik-Event.
...
* **Gold als verlässlicher "Boden-Indikator":** Die wichtigste Erkenntnis ist das Timing der Erholung. **In 100% der betrachteten Crashes (6 von 6)** erreichte Gold seinen absoluten Tiefpunkt und drehte wieder nach oben, **bevor** der S&P 500 (SPY) seinen Tiefpunkt fand (Spannweite: von 5 Tagen bis zu 601 Tagen Vorlauf).
...
* **Central Bank Policy Error (DFF vs T10YIE vs DXY):** Das mathematisch perfekte Kaufsignal für Gold (Fiat-Flucht). Wenn die FED über ein 60-Tage-Fenster panisch die Zinsen senkt (DFF sinkt > 0.25%), der Anleihemarkt aber gleichzeitig steigende Inflationserwartungen einpreist (T10YIE steigt > 0.10%), verliert das Fiat-System massiv an Vertrauen. Der Indikator triggert **nur dann**, wenn der US-Dollar (DXY) im selben Zeitraum nicht stark aufwertet (<= +2,0%), da ein starker Dollar den Gold-Run blockiert.
  * *Statistische Realität:* Mit dem DXY-Filter stieg Gold in 73,6% dieser Phasen an. Die durchschnittliche 60-Tage-Rendite für Gold in diesen Mega-Runs beträgt extreme **+7,95%**.
...
* **Das TGA-Konto (Treasury General Account):** Das Konto des Finanzministeriums wirkt als reiner Liquiditäts-Sauger.
  * *Inflations-Crash 2022:* Das TGA stieg in den 3 Monaten vor dem Aktien-Peak massiv von 132 Mrd. $ auf 366 Mrd. $ an und entzog dem Markt somit gezielt 234 Mrd. $ an Liquidität.
...
* **Zinsstrukturkurve (Spread 10Y-2Y):** Das klassische "Un-inverting".
  * *Dotcom (2000) & Finanzkrise (2008):* Die Kurve war lange invers (unter 0). Erst in den 3 Monaten vor dem Absturz stieg der Spread wieder massiv an (die Kurve "ent-inversierte" sich). Der tatsächliche Aktien-Crash folgte exakt mit der Normalisierung der Zinsstruktur.
* **Sahm Rule (Reale Rezession):** Steigt die Arbeitslosigkeit zu schnell (Wert > 0.50), gilt eine Rezession als unausweichlich.
...
* **Die wahre Stärke (Der Trend-Filter):** Seine absolute Glanzleistung zeigt das LSTM in den tiefen Bärenmärkten (`DOWNTRENDS`). Es lässt sich von aggressiven Zwischen-Rallyes ("Dead Cat Bounces") nicht täuschen und blockiert Kauf-Signale. Umgekehrt identifiziert es den Beginn von Jahrhundert-Böden extrem zuverlässig (z. B. Corona-Crash Bottom zu 79 % getroffen), wenn der RSI historische Extreme erreicht.
...
* **⚠️ Aktueller Integrations-Status:** Während das Bitcoin-Netzwerk (`btc_regime_v1`) bereits vollständig live in die `IndicatorEngine` und in das Benachrichtigungssystem (NTFY) integriert ist, liegen die Gewichte des frisch trainierten Tech-Netzwerks (`qqq_regime_v1`) aktuell nur im Modell-Ordner vor. Das QQQ-Netzwerk muss in einem zukünftigen Entwicklungs-Schritt noch offiziell in die Live-Engine und die Alarm-Logik eingebaut werden.
