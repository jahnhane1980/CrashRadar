...
* **[TODO] BEWEIS ANTRETTEN:** Es existiert aktuell **noch kein Backtest-Skript**, das diese These quantitativ belegt! Wir müssen dringend ein entsprechendes Proof-Skript (z.B. `scratch/analyse/Tech-Infrastruktur-Rotation.js`) bauen, das historische 13F-Daten, DIX-Divergenzen und VandaTrack/Odd-Lots mit Tech-Top-Bildungen korreliert, um diesen institutionellen "Stealth-Exit" empirisch nachzuweisen.
...
* **Reine Krypto-Proxies (MSTR & COIN) als Zyklus-Indikator:**
  ...
  * **[TODO] BEWEIS ANTRETTEN:** Es existiert aktuell **noch kein Backtest-Skript**, das den historischen Vorlauf/Nachlauf von MSTR/COIN gegenüber BTC (Tage bis zum Peak/Bottom) und die harten SMA-200/SMA-50-Regeln empirisch auswertet und belegt.
...
* **Die wahre Stärke (Der Trend-Filter):** Seine absolute Glanzleistung zeigt das LSTM in den tiefen Bärenmärkten (`DOWNTRENDS`). Es lässt sich von aggressiven Zwischen-Rallyes ("Dead Cat Bounces") nicht täuschen und blockiert Kauf-Signale. Umgekehrt identifiziert es den Beginn von Jahrhundert-Böden extrem zuverlässig (z. B. Corona-Crash Bottom zu 79 % getroffen), wenn der RSI historische Extreme erreicht. **[TODO] BEWEIS ANTRETTEN: Ein Backtest-Skript fehlt, das diese 79% Trefferquote am Corona-Boden für das LSTM-Modell systematisch belegt.**
...
3. **Preis-Action: "Verkäufer-Erschöpfung" (Bullish Divergence)**
   ...
   * **[TODO] BEWEIS ANTRETTEN:** Es existiert aktuell **noch kein Backtest-Skript**, das exakt diese Kombination (CBOE Volumen > 1.5x + VIX > 35 + RSI Divergence) über alle historischen Crashes testet und die punktgenauen Bodentreffer belegt.
...
* **Die Überprüfung:** **[TODO] BEWEIS ANTRETTEN:** Ein Backtest über 20 Jahre, der das harte "All-in/All-out" gegen ein Fractional-Kelly-Modell antreten lässt, welches das Exposure je nach Anzahl der feuernden Indikatoren stufenweise reduziert (z.B. 80%, 60%, 20%), muss noch programmiert werden.
...
* **Die Überprüfung:** **[TODO] Der historische Beweis für diese These muss noch angetreten werden!** Es fehlt noch ein Backtest-Skript, das empirisch belegt, dass Breakouts mit einem RVOL >= 2.0 tatsächlich eine signifikant höhere Erfolgsquote / Performance aufweisen als Breakouts mit einem normalen RVOL.
